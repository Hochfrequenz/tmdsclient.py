...
class TestTmdsAnschlussobjekt:
    async def test_anschlussobjekt_set_plattform(self, tmds_client_with_default_auth):
        ...
        internal_ao_id = str(uuid.uuid4())
        event_id = str(uuid.uuid4())
        with aioresponses() as mocked_tmds:
            mocked_tmds.get(
                f"{tmds_settings.server_url}api/Anschlussobjekt/allIds",
                status=200,
                payload={"Anschlussobjekt": [{"interneId": internal_ao_id, "externeId": external_ao_id}]},
            )
            mocked_tmds.post(
                # pylint:disable=line-too-long
                f"{tmds_settings.server_url}api/Anschlussobjekt/{external_ao_id}/setPlattform?plattformfaehig=true&aenderungsdatum=2000-01-01T00%253A00%253A00%252B00%253A00",
                status=200,
            ...
            actual = await client.set_plattformfaehigkeit(
                internal_ao_id, is_plattformfaehig=True, change_date=datetime(2000, 1, 1, tzinfo=timezone.utc)
            )
